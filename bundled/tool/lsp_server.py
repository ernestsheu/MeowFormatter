...
LSP_SERVER = server.LanguageServer(
    name="MeowCodeFormatterServer", version="1.0.0", max_workers=MAX_WORKERS
)
...
# Minimum version of Meow PEP supported.
MIN_VERSION = "0.0.1"
...
def _log_version_info() -> None:
    for value in WORKSPACE_SETTINGS.values():
        try:
            ...
            log_to_output(
                f"Error while detecting MeowCodeFormatter version:\r\n{traceback.format_exc()}"
            )
...
def _workaround_for_reload_issue():
    # workaround for reload
    lib_path = os.fspath(pathlib.Path(__file__).parent.parent / "tool-libs")
...
def _run_tool_on_document(
    ...
    if use_stdin:
        exclude_arg, remaining_arg_list = _parse_exclude_arg(argv)
...
def _parse_exclude_arg(argv: list(str)):
...
def log_to_output(
    ...
    """Logs messages to Output > Meow PEP channel only."""
